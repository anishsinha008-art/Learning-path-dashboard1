...
if "active_section" not inimport streamlit as st
import pandas as pd
import plotly.graph_objects as go
import numpy as np
import os
...
# ------------------ MENU PANEL HTML ------------------
menu_html = """
<div class="menu-panel {open_class}">
...
for section in [
    ...
    menu_html += f'<button class="menu-button {active_class}" onclick="fetch(\'/_set_section?section={section}\')">{section}</button>'
menu_html += "</div>"

# Toggle open/closed class
menu_html = menu_html.replace("{open_class}", "open" if st.session_state.menu_open else "")
